document.addEventListener('DOMContentLoaded', () => {
    ...
    animateElements.forEach((el, index) => {
        ...
        (el as HTMLElement).style.transition = `opacity 0.8s ease ${index * 0.1}s, transform 0.8s ease ${index * 0.1}s`;
        observer.observe(el);
    });
    ...
    const statsObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                const numberElement = entry.target.querySelector('.tech-stat-number') as HTMLElement;
